...
class QuestionTreeCategory extends React.Component {
  ...
  searchDatabaseInternally(category, searchInput) {
    if (category.name === '') {
      return category.children.map(c => this.searchDatabaseInternally(c))
    ...
    let next = category.children.map(c => this.searchDatabaseInternally(c, searchInput))
      .reduce((l, r) => l.concat(r), []);
    if (jsx) {
  ...
  searchQuestionsInternally(category, searchInput) {
    const matchingQuestions = category.questions.filter(q => {
      return q.question.toLowerCase()
        ...
        || q.id.toLowerCase()
          .includes(searchInput)
        || q.answers.filter(a => a.toLowerCase()
          .includes(searchInput)).length > 0;
    })
      .map(q => {
