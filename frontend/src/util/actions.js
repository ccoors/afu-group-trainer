...
function startCountdown(time) {
  return {
    action: UserActions.START_COUNTDOWN,
    time: time,
  };
}
...
function resetCountdown() {
  return {
    action: UserActions.RESET_COUNTDOWN,
  };
}
...
function updateState(action, setState, socket) {
  switch (action.action) {
    ...
    case UserActions.DELETE_QUESTION_LIST:
      ...
      break;
    case UserActions.START_COUNTDOWN:
      socket.send(JSON.stringify({
        StartCountdown: {
          time: action.time
        }
      ...
      break;
    case UserActions.RESET_COUNTDOWN:
      socket.send(JSON.stringify('StopCountdown'));
      break;
    default:
      setState({
